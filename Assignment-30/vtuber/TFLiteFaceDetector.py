...
class UltraLightFaceDetecion:
    def __init__(
        ...
        self._get_boxes_tensor = partial(
            self._interpreter.get_tensor, output_details[0]["index"]
        )
        self._get_scores_tensor = partial(
            self._interpreter.get_tensor, output_details[1]["index"]
        )
    ...
    def _pre_processing(self, img):
        ...
        cv2.normalize(
            image_norm, image_norm, alpha=-1, beta=1, norm_type=cv2.NORM_MINMAX
        )
    ...
    def _decode_regression(self, reg):
        # bounding box regression
        center_xy = (
            reg[:, :2] * self._center_variance * self._anchors_wh + self._anchors_xy
        )
